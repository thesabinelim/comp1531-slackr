...
# Return u_id of user associated with token.
# Raise InvalidTokenError exception if token has been invalidated.
# Raise CounterfeitTokenError exception if token was not generated by server.
def validate_token(token):
    ...
    try:
        ...
        raise CounterfeitTokenError(description="Token was not generated by server!")
    u_id = payload['u_id']
    user = db_get_user_by_u_id(u_id)
    if not user.has_token(token):
        raise InvalidTokenError(description="Token was invalidated!")
    return payload['u_id']

# Invalidate a provided token so all future authorisation attempts with token
# fail.
# Raise InvalidTokenError exception if token has already been invalidated.
...
def auth_login(email, password):
    ...
    if not user:
        raise ValueError(description="No user with that email exists!")
